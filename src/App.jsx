...
import { 
  ...
  ChevronLeft, ChevronRight, Type, Image as ImageIcon, Upload
} from 'lucide-react';
...
export default function App() {
  ...
  const handleMouseMove = (e) => {
    ...
    // Boundary clamping for print zone
    setLabConfig(prev => ({ 
      ...prev, 
        ...
        <div className="max-w-7xl mx-auto w-full grid lg:grid-cols-12 gap-12 items-center relative z-10">
          <div className="lg:col-span-7 space-y-8 z-20">
            ...
            <div className="flex gap-4">
              <button className="bg-[#1e1e1e] text-[#f5f2eb] px-10 py-5 rounded-2xl font-black uppercase tracking-widest text-[11px] flex items-center gap-2 group shadow-xl">
          ...
          <div className="lg:col-span-5 relative h-[500px] md:h-[600px]">
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 z-30">
              <button onClick={prevSlide} className="w-10 h-10 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white"><ChevronLeft size={20} /></button>
              <button onClick={nextSlide} className="w-10 h-10 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white"><ChevronRight size={20} /></button>
            </div>
          </div>
        </div>
      </section>

      {/* Custom Workshop Page - NEW 30/70 SPLIT */}
      <section id="customlab" className="py-24 bg-[#1e1e1e] text-[#f5f2eb] px-6 overflow-hidden">
        <div className="max-w-[1440px] mx-auto">
          <div className="grid lg:grid-cols-10 gap-16 items-start">
            ...
            {/* RIGHT SIDE: T-SHIRT VISUALIZATION (70%) */}
            <div className="lg:col-span-7 flex items-center justify-center min-h-[700px] relative">
              <div className="relative w-full max-w-2xl aspect-square flex items-center justify-center p-12">
                ...
                {/* T-Shirt Base Image Mockup */}
                <div className="absolute inset-0 w-full h-full flex items-center justify-center pointer-events-none">
                  {/* We use a realistic tee image here */}
                  <div 
                ...
                <div className="relative w-72 h-96 border border-white/5 z-10 flex items-center justify-center group">
                  ...
                  {labConfig.type === 'text' ? (
                    ...
                    >
                      {labConfig.customImage ? (
                        <img src={labConfig.customImage} className="w-full h-auto pointer-events-none drop-shadow-2xl" alt="Custom Design" />
                      ) : (
                        <div className="p-8 text-center text-[10px] font-black uppercase tracking-[0.3em] text-stone-700 opacity-20">
                          Waiting for Upload
                        </div>
                      )}
                    </div>
                  ...
                  <div className="absolute top-0 left-0 w-4 h-4 border-t border-l border-stone-800 opacity-20"></div>
                  <div className="absolute top-0 right-0 w-4 h-4 border-t border-r border-stone-800 opacity-20"></div>
                  <div className="absolute bottom-0 left-0 w-4 h-4 border-b border-l border-stone-800 opacity-20"></div>
                  <div className="absolute bottom-0 right-0 w-4 h-4 border-b border-r border-stone-800 opacity-20"></div>
                </div>

                {/* Dynamic Labels */}
                <div className="absolute top-20 right-20 space-y-2 opacity-30 text-right">
                  <p className="text-[10px] font-black uppercase tracking-widest">Architectural Visualization</p>
                  <p className="text-[10px] font-bold">Safe Print Zone: 12x16in</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16">
            {PRODUCTS.filter(p => activeCategory === "All" || p.category === activeCategory).map(product => (
              <div key={product.id} className="group cursor-pointer animate-fadeIn">
                <div className="aspect-[3/4] bg-[#ece9e0] rounded-[2rem] overflow-hidden mb-6 relative">
                  ...
                  <div className="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-6">
                     <button onClick={() => addToCart(product)} className="w-full bg-white py-4 rounded-xl font-black text-[10px] uppercase shadow-xl transform translate-y-4 group-hover:translate-y-0 transition-transform">Quick Add</button>
                  </div>
                </div>
                <div className="flex justify-between items-start px-2">
                  ...
                  <span className="font-black text-lg text-stone-500 font-space">₹{product.price}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-4 gap-16 mb-20">
            <div className="md:col-span-2 space-y-8">
              ...
              <p className="text-stone-500 max-w-sm text-sm leading-relaxed font-medium italic">Redefining the standard of basic apparel through meticulous engineering and organic neutral aesthetics.</p>
              <div className="flex gap-4">
            ...
            <div>
              ...
              <ul className="space-y-4 font-bold uppercase text-[11px] tracking-widest text-stone-400">
                <li><a href="#shopall" className="hover:text-white">Shop All</a></li>
                <li><a href="#customlab" className="hover:text-white">Custom Lab</a></li>
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex justify-end">
          <div className="w-full max-w-md bg-[#fdfcf8] h-full flex flex-col shadow-2xl animate-slideLeft">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8 scrollbar-hide">
              {cart.map(item => (
                <div key={item.cartId} className="flex gap-6 items-start">
